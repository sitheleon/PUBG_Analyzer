...
#PyQt 로 구현 GUI
class PUBGAnalyzerApp(QWidget):
    def __init__(self, api_key):
        super().__init__()
        self.analyzer = PUBGPlayerAnalyzer(api_key)
        self.initUI()

    def initUI(self):
        self.setWindowTitle("PUBG 플레이어 분석기")
        self.setGeometry(300, 300, 400, 200)

        layout = QVBoxLayout()


        self.label = QLabel("플레이어 이름 입력:")
        layout.addWidget(self.label)

        self.playerNameInput = QLineEdit()
        layout.addWidget(self.playerNameInput)

        self.analyzeButton = QPushButton("분석")
        self.analyzeButton.clicked.connect(self.analyzePlayer)
    ...
    def analyzePlayer(self):
        player_name = self.playerNameInput.text()
        player_data = self.analyzer.analyzePlayer(player_name)

        if not player_data.empty:

            top10_win_rate_summary = self.analyzer.calculateTop10WinRateByPlaystyle(player_data)
            highest_win_rate_playstyle = top10_win_rate_summary["top10_win_rate"].idxmax()
            highest_win_rate = top10_win_rate_summary["top10_win_rate"].max()

            # 결과 표시
            self.resultLabel.setText(f"플레이어: {player_name}\n"
                                     f"최고 승률 플레이스타일: {highest_win_rate_playstyle}\n"
                                     f"승률: {highest_win_rate:.2%}")
        else:
            self.resultLabel.setText("플레이어 데이터를 찾을 수 없습니다.")




if __name__ == '__main__':
    api_key = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI0MGY2Mzg1MC02YzM0LTAxM2QtNDRjNC0xNjVmNmJlZjNhYmMiLCJpc3MiOiJnYW1lbG9ja2VyIiwiaWF0IjoxNzI4ODk0NTI2LCJwdWIiOiJibHVlaG9sZSIsInRpdGxlIjoicHViZyIsImFwcCI6InB1Ymdfc3Vydml2YWxfIn0.evM3arqRlKF9j8o3nIyq-lTedu3k0FtS1Yy1gdB_-ck"  # 실제 API 키로 교체
    app = QApplication(sys.argv)
    ex = PUBGAnalyzerApp(api_key)
    ex.show()
    sys.exit(app.exec_())
